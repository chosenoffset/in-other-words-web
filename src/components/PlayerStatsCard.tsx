...
export function PlayerStatsCard() {
  ...
  const StatBox = ({
    ...
    const animatedValue = useAnimatedCounter(isAnimated ? numericValue : numericValue, 1200)
    const displayValue = typeof value === 'number' ? animatedValue : value

    const variantStyles = {
      default: 'bg-gradient-to-br from-slate-50 to-gray-100 dark:from-slate-800/50 dark:to-gray-800/50 border-slate-200 dark:border-slate-700',
      success: 'bg-gradient-to-br from-emerald-50 to-green-100 dark:from-emerald-900/20 dark:to-green-900/20 border-emerald-200 dark:border-emerald-700',
      primary: 'bg-gradient-to-br from-sky-50 to-blue-100 dark:from-sky-900/20 dark:to-blue-900/20 border-sky-200 dark:border-sky-700',
      warning: 'bg-gradient-to-br from-amber-50 to-orange-100 dark:from-amber-900/20 dark:to-orange-900/20 border-amber-200 dark:border-amber-700'
    }
    ...
    return (
      ...
      >
        ...
        <div className={`text-3xl font-bold mb-2 group-hover:scale-105 transition-transform duration-300 ${textStyles[variant]}`}>
          {isAnimated && typeof value === 'number' ? animatedValue : displayValue}
        </div>
          ...
          {stats && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <StatBox
                ...
                icon="📊"
                delay={200}
                variant={stats.winRate >= 70 ? 'success' : stats.winRate >= 50 ? 'warning' : 'default'}
                isAnimated={false}
              />
              ...
              {(() => {
                  ...
                  <div className="col-span-full relative">
                    <div className="blur-sm">
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                        <StatBox value="???" label="Current Streak" icon="🔥" variant="warning" isAnimated={false} />
                        <StatBox value="???" label="Best Streak" icon="⭐" variant="success" isAnimated={false} />
                        <StatBox value="???" label="Avg Guesses" icon="🎯" variant="primary" isAnimated={false} />
                        <StatBox value="???" label="Avg Time" icon="⏱️" variant="default" isAnimated={false} />
                        <StatBox value="???" label="Best Time" icon="⚡" variant="warning" isAnimated={false} />
                      </div>
                    </div>
                    <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6 bg-gradient-to-br from-white/95 to-gray-50/95 dark:from-gray-900/95 dark:to-gray-800/95 rounded-lg backdrop-blur-sm border border-border">
                      ...
                      <p className="text-foreground font-semibold mb-2">Premium Statistics</p>
                      <p className="text-muted mb-4 max-w-sm">
                        Unlock detailed streaks and timing analytics with a subscription.
                      </p>
                      <SubscribeButton />
                    </div>
                  </div>
                ...
                return (
                  <>
                    <StatBox
                      value={stats.currentStreak !== undefined && stats.currentStreak !== null ? stats.currentStreak : 0}
                      label="Current Streak"
                      icon="🔥"
                      delay={300}
                      variant={stats.currentStreak !== undefined && stats.currentStreak !== null && stats.currentStreak >= 5 ? 'warning' : 'default'}
                    />
                    <StatBox
                      value={stats.longestStreak !== undefined && stats.longestStreak !== null ? stats.longestStreak : 0}
                      label="Best Streak"
                      icon="⭐"
                      delay={400}
                      variant="success"
